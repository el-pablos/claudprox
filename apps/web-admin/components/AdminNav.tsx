    ...
    <header className="sticky top-0 z-40 border-b border-ctos-border bg-ctos-panel/80 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden items-center gap-4 text-sm text-slate-300 sm:flex" aria-label="Navigasi admin">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.href}
                href={item.href}
    ...
    <div className="sm:hidden">
      ...
      >
        <div className="flex items-center justify-between border-b border-ctos-border px-4 py-3">
          <span className="font-mono text-sm text-ctos-accent">
            ClaudProx <span className="text-xs uppercase tracking-wider text-red-400/70">[ADMIN]</span>
          </span>
          <button
            type="button"
        ...
        <nav className="flex flex-col gap-1 p-3" aria-label="Navigasi admin mobile">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.href}
              href={item.href}
